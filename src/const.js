...
const levelDataInfoRules = () => {
    try {
        ...
        var gpass = siteinfo.galaxyPassSet.sort((a, b) => (a.id > b.id ? 1 : -1))[0];
        return [
            {
                name: "gPass",
...
export const gameDataMain = "poker,wheel,bet,backgammon".split(",");
export const gameDataMainName = "Poker,Online Wheel,Bet,Backgammon".split(",");
export const gameDataMainCode = "poker,wheel,bet,backgammon".split(",");
export const gameData = "boom,blackjack,blackjack3,roulette,roulette3d,baccarat,deuceswild,vpjacks,highlow,slotlucky,slotfruits,slotramses,slotarabian,slotsoccer,slotspace".split(",");
export const gameDataName = "BoOoOoM,BlackJack,BlackJack3,Roulette,Roulette 3D,Baccarat,Deuces Wild,Jacks or Better,High Low,Slot Lucky Christmas,Slot Fruits,Slot Ramses,Slot Arabian,Slot Soccer,Slot Space".split(",");
    ...
    if (!siteInfo?.cashoutLimitDollar) return false;
    {
        cashoutData.map((game) => {
            ...
            _games.push({
                label: game.text,
                title: game.text,
                helper: game.limit.replace("$100 ", "$" + siteInfo?.cashoutLimitDollar + " "),
                getwaykey: game.getwaykey,
                bonus: game.bonus,
                icon: game.icon,
...
export const updateBonusLabel = (bonusOld, loginToken, siteInfo) => {
    ...
    try {
        var _lvl = bonus.text.split(" ")[1].replace("Bonus", "").replace("Gift", "");
    } catch (error) {
        var _lvl = "1";
    }
...
export const haveModerator = (userTags) => {
    ...
    if (userTags) {
        userTags.map(function (tag) {
            if (tag.name == "ROLE_MODERATOR" || tag.name == "ROLE_ADMIN" || tag.name == "ROLE_ROOT") {
                isAdmin = true;
            }
        });
    }
...
export const haveOperator = (userTags) => {
    ...
    if (userTags) {
        userTags.map(function (tag) {
            if (tag.name == "ROLE_OPERATOR" || tag.name == "ROLE_MODERATOR" || tag.name == "ROLE_ADMIN" || tag.name == "ROLE_ROOT") {
                isAdmin = true;
            }
        });
    }
...
export const siteInfoDef = {
    ...
    updateday: "2024-07-07T22:51:39.224Z",
};
