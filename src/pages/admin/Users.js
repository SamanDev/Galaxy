...
import { Input, Segment, Button, Dimmer, Loader, Icon, Modal, Grid, Label, Dropdown, Checkbox, FormInput, FormGroup, FormCheckbox, Form } from "semantic-ui-react";
...
import { haveAdmin, haveModerator, haveRoot, doCurrency, levelDataInfo, doCurrencyMil, levelClassInside } from "../../const";
...
const lvlfilteroptions = [];
lvlfilteroptions.push({ key: "100", text: "No Filter", value: 100 });
lvlfilteroptions.push({ key: "5", text: "5Lvl", value: 5 });
lvlfilteroptions.push({ key: "10", text: "10Lvl", value: 10 });
lvlfilteroptions.push({ key: "15", text: "15Lvl", value: 15 });
...
const FilterComponent = ({ filterText, onFilterOk, onFilter, onClear, setExMode }) => (
    <>
        <Input icon="search" placeholder="Search..." id="search" type="text" aria-label="Search Input" value={filterText} onChange={onFilter} onBlur={onFilterOk} />
    </>
);
...
const setGiftAmount = (level) => {
    if (level >= levelDataInfo[4].minLevel) {
        var g = generateRandomInteger(levelDataInfo[4].minAmount, levelDataInfo[4].maxAmount);
    } else if (level >= levelDataInfo[5].minLevel) {
        var g = generateRandomInteger(levelDataInfo[5].minAmount, levelDataInfo[5].maxAmount);
    } else {
        var g = generateRandomInteger(levelDataInfo[6].minAmount, levelDataInfo[6].minAmount * (level + 1));
    }
...
function genWALink(number, user) {
    ...
    var txt = "با سلام (" + user.username + ") عزیز %0A %0A %0A %0A لینک ورود یکبار مصرف:%0A" + genLink(user);
    //url = "http://localhost:3000";
    return (
        <>
            <a href={"https://api.whatsapp.com/send?phone=" + num + "&text=" + txt} target="_blank">
...
function Admin(prop) {
    ...
    const [lvlFilter, setLvlFilter] = useState(5);
    ...
    const handleFilterLvlSearch = (e, { value }) => {
        setLvlFilter(value);
    };
    ...
    const columnsDownLine = [
...

        {
            ...
            format: (row) => (
                <>
                    <span className="msglink fw-bold" onClick={() => prop.addTabData(row.username, prop.getwaysList)}>
                        {row.username}
                    </span>
                </>
            ),
...

        {
            ...
            format: (row) => (
                <>
                    <CheckboxToggle check={row.userBlock} user={row} userkey="userBlock" onChange={updateUserObj} />
                </>
            ),
            sortable: true,
        },
    ...
    var columns = [
        ...
        },
        {
            ...
            format: (row) => (
                <>
                    <LevelIcon level={row.level} text="" mode="levels" classinside={levelClassInside(row.level)} number="" width="40px" />
                </>
            ),
            sortable: true,
        },
        {
            ...
            format: (row) => <>{haveAdmin(loginToken.roles) && row.bankInfos.length > 0 && <>{genWALink(row.bankInfos[0].mobile, row)}</>}</>,
            sortable: true,
        },
        {
            name: "username",
...

        {
            ...
            format: (row) => (
                <>
                    {row.refer && (
                        <span className="msglink fw-bold " onClick={() => prop.addTabData(row.refer, prop.getwaysList)}>
                            {row.refer}
                        </span>
                    )}
                </>
            ...
            format: (row) => (
                <>
                    {row.totalCashout - row.totalDeposit != 0 && (
                        <Label size="tiny" color={row.totalCashout - row.totalDeposit > 0 ? "green" : "red"}>
        ...
        },
        {
            ...
            format: (row) => <>{(row.giftPlaySecond * 100) / siteInfo.secondForGift > 100 ? 100 : parseFloat((row.giftPlaySecond * 100) / siteInfo.secondForGift).toFixed(2)}</>,
            sortable: true,
        },
        {
            name: "vip",
            selector: (row) => row.vipPlaySecond,
            format: (row) => <> {(row.vipPlaySecond * 100) / siteInfo.secondForGift > 100 ? 100 : parseFloat((row.vipPlaySecond * 100) / siteInfo.secondForGift).toFixed(2)}</>,
            sortable: true,
        },
    ...
    const fetchUsers = async (page) => {
        ...
        setLoading(true);
        try {
            const res = await adminGetService(`getUsersByAdmin?name=${_name}&value=${_val}&page=${page}&number=${perPage}&login=${dataLoginDay}&contain=${_contain}`);
            if (res.status === 200) {
        ...
        var newSelect = [];
        {
            selectedRows.map((user, i) => {
                if (!user?.multiAccount && user?.refer != "runner" && user?.refer != "bots") {
                    ...
                    newUser.amount2 = parseFloat(setGiftAmount(user.level) / getRate).toFixed(2);
    ...
    if (haveAdmin(loginToken.roles) && 1 == 2) {
        columns.push(
            ...
            },
            {
                ...
                format: (row) => <CheckboxToggle check={haveAdmin(row.roles)} user={row} userkey="Roles" onChange={updateUserObj} />,
                sortable: true,
            },
            {
                name: "Moderator",
                selector: (row) => row.roles,
                format: (row) => <CheckboxToggle check={haveModerator(row.roles)} user={row} userkey="Roles" onChange={updateUserObj} />,
                sortable: true,
            }
        );
    }
    ...
    const subHeaderComponentMemo = React.useMemo(() => {
        ...
        return (
            <>
                <Grid verticalAlign="middle" doubling columns={2} centered as={Segment} color="red">
                    <Grid.Row>
                        <Grid.Column>
                        <Form unstackable>
                                <FormGroup widths={1}>
                                ...
                                <>
                                    ...
                                    {haveAdmin(loginToken.roles) && (
                                        <>
                                            <Button onClick={() => prop.addGatewayTabData("Gateways")}>Gateways</Button>
                                            <Button color="blue" className="float-end" onClick={() => setCashierOpen(true)}>
                                                Cashier
                                            </Button>
                                            <Button color="orange" className="float-end" onClick={() => setOverCashierOpen(true)}>
                                                OverCashout
                                            </Button>
                                        </>
                                    )}

                                    <Button onClick={() => prop.addMainTabData("Winners")}>Winners</Button>
                            ...
                            <Form unstackable>
                                <FormGroup widths={3}>
                                    <FormInput label="Search Levels">
                                        ...
                                        {dataSearch.toString().indexOf("up") > -1 && <Dropdown placeholder={lvlfilteroptions[1].text} floated="right" selection options={lvlfilteroptions} onChange={handleFilterLvlSearch} defaultValue={lvlFilter} fluid />}
                                    </FormInput>
                                    <FormInput label="NoLogin Days">
                                        <Filter onFilter={handleChangeLogin} value={dataLoginDay} mymode="login" />
                                    </FormInput>
    ...
    }, [filterText, data, selectedList, dataLoginDay, dataSearch, dataSortedID, perPage]);
    const subHeaderComponentMemoDown = React.useMemo(() => {
        ...
        return (
            <>
                <Grid verticalAlign="middle" columns={2} centered as={Segment} color="red">
                    <Grid.Row>
                        <Grid.Column>
                            <Button className="float-end" color="red" onClick={() => fetchUsers(1)}>
                                Search
                            </Button>
                        </Grid.Column>
    ...
    const CustomLoader = () => (
        <>
            <Segment basic style={{ height: "calc(100vh - 150px)", overflow: "auto" }}>
                <Dimmer active inverted>
    ...
    return (
        <>
            ...
            <Modal onClose={() => setFirstOpen(false)} onOpen={() => setFirstOpen(true)} open={firstOpen} size="large" style={{ height: "auto" }}>
                <AddGift selectedList={selectedList} />
            </Modal>
            <Modal onClose={() => setMailOpen(false)} onOpen={() => setMailOpen(true)} open={mailOpen} size="large" style={{ height: "auto" }}>
                <SendMail selectedList={selectedList} {...prop} />
            </Modal>
            <Modal onClose={() => setNotOpen(false)} onOpen={() => setNotOpen(true)} open={notOpen} size="large" style={{ height: "auto" }}>
